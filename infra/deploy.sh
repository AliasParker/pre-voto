...
# Load PUBLIC_SITE_URL from .env if available
if [ -f "$APP_DIR/.env" ]; then
  PUBLIC_SITE_URL=$(grep -E '^PUBLIC_SITE_URL=' "$APP_DIR/.env" | cut -d= -f2- || echo "")
fi
HEALTH_URL="${PUBLIC_SITE_URL:-https://localhost}/api/health"
MAX_WAIT=60
HEALTH_INTERVAL=2
...
if [ "$HTTP_CODE" = "200" ]; then
  ...
  echo "  3. Check DNS:          dig YOUR_DOMAIN +short"
