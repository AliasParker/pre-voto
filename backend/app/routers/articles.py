...
def _verify_preview_token(token: str | None) -> None:
    if not settings.admin_preview_token or not token:
        raise HTTPException(status_code=401, detail="Preview token required")
    if token != settings.admin_preview_token:
...
# Preview routes must be defined BEFORE the {slug} route
@router.get("/{country}/preview", response_model=list[ArticleOut])
async def list_articles_preview(
    response: Response,
    country_election: tuple[Country, Election] = Depends(get_country_with_election),
    db: AsyncSession = Depends(get_db),
...
@router.get("/{country}/preview/{slug}", response_model=ArticleDetail)
